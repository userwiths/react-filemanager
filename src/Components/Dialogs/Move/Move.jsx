...
import {
  ...
  enterToPreviousDirectorySublist,
  moveItems
  ...
  render() {
    ...
    return (
      <Dialog
        open={open}
        onClose={handleClose}
        aria-labelledby="form-dialog-move"
        fullWidth={true}
        maxWidth={"sm"}
      >
        <form>
          <DialogTitle id="form-dialog-move">
            Move files to{" "}
            <small style={{ color: "grey" }}>{selectedPath.join("/")}</small>
          </DialogTitle>
          <DialogContent>
            <FileListSublist />
          </DialogContent>
          <DialogActions>
            ...
            <Button
              color="primary"
              onClick={e => handleSave(e, selectedFiles)}
              disabled={!canMove}
              type="submit"
            >
              Move
...
const mapStateToProps = state => {
  ...
  const canMove =
    state.path.join("") !==
    state.pathSublist.join("") +
      (state.selectedFolderSublist ? state.selectedFolderSublist.name : "");

  return {
    open: state.visibleDialogMove,
    selectedFolderSublist: state.selectedFolderSublist,
    selectedPath: state.selectedFolderSublist
      ? [...state.pathSublist, state.selectedFolderSublist.name]
      : [],
